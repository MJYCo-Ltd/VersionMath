...
CMatrix CQuaternion::GetMatrix() const
{
    CMatrix matTmp(3,3);
    ...
    /// 归一化
    dLength = 1.0/dLength;
    double dX=m_dX*dLength,
           dY=m_dY*dLength,
           dZ=m_dZ*dLength,
           dS=m_dS*dLength;

    /// 构建旋转矩阵
    matTmp(0,0) = 1.0 - 2.0*(dY* dY+ dZ* dZ);
    matTmp(0,1) = 2.0*( dX* dY+ dZ* dS);
    matTmp(0,2) = 2.0*( dX* dZ- dY* dS);
    matTmp(1,0) = 2.0*( dX* dY- dZ* dS);
    matTmp(1,1) = 1.0 - 2.0*( dX* dX+ dZ* dZ);
    matTmp(1,2) = 2.0*( dY* dZ+ dX* dS);
    matTmp(2,0) = 2.0*( dX* dZ+ dY* dS);
    matTmp(2,1) = 2.0*( dY* dZ- dX* dS);
    matTmp(2,2) = 1.0 - 2.0*( dX* dX+ dY* dY);
